...
<?php if (isset($_GET['id']) && isset($_GET['mentor_id'])) : ?>
    <?php
    $id = $_GET['id'];
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="mentor_id" id="" class="form-control">
                        ...
                        $resultGetAllMentor = mysqli_query($conn, $getAllMentorSql);
                        if ($resultGetAllMentor) {
                            $countMentor = mysqli_num_rows($resultGetAllMentor);
                            if($countMentor > 0){
                                $dataMentor = mysqli_fetch_all($resultGetAllMentor, MYSQLI_ASSOC);
                            }
                        }
                        ?>
                        <?php if($dataMentor != null): ?>
                            <?php foreach($dataMentor as $row): ?>
                                <option value="<?php echo $row['id']; ?>" <?php echo $row['id'] == $data['mentor_id'] ? 'selected' : '' ?>><?php echo $row['fullname']; ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                </div>
...
if(isset($_POST['submit'])){
    ...
    if($resultDeleteRelationCourse) {
        $insertRelationCourseSql = "insert into user_course (user_id, course_id, type) values ('$mentor', '$id', 1)";
